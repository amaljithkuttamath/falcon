...
def get_primer(df, df_name):
    # Generate a primer description and code for plotting with matplotlib using a dataframe
    columns_desc = "', '".join(df.columns)
    primer_desc = f"Use a dataframe called {df_name} from 'data_file.csv' with columns '{columns_desc}'."

    # Add details about categorical and numeric columns
    for col in df.columns:
        unique_values = df[col].drop_duplicates()
        if len(unique_values) < 20 and df.dtypes[col] == "O":
            primer_desc += f"\nThe column '{col}' has categorical values '{', '.join(unique_values)}'."
        elif df.dtypes[col] in ["int64", "float64"]:
            primer_desc += f"\nThe column '{col}' is type {df.dtypes[col]} and contains numeric values."

    primer_desc += "\nLabel the x and y axes appropriately. Using Python version 3.9.12, create a script using the dataframe to graph the following:"

    # Script template for plotting
    pimer_code = f"""import pandas as pd
import matplotlib.pyplot as plt
